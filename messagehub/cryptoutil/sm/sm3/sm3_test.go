...
func TestGolden(t *testing.T) {
	for i := 0; i < len(golden); i++ {
		g := golden[i]
		s := fmt.Sprintf("%x", cryptoutil.SumSM3([]byte(g.in)))
		if s != g.out {
...
func TestPnPanic(t *testing.T) {
		...
		cryptoutil.SumSM3(buf)
}


func TestSize(t *testing.T) {
	c := cryptoutil.New()
	if got := c.Size(); got != cryptoutil.Size {
		t.Errorf("Size = %d; want %d", got, cryptoutil.Size)
	}
}

func TestBlockSize(t *testing.T) {
	c := cryptoutil.New()
	if got := c.BlockSize(); got != cryptoutil.BlockSize {
		t.Errorf("BlockSize = %d want %d", got, cryptoutil.BlockSize)
	}
}

var bench = cryptoutil.New()
