...
func TestSignVerify(t *testing.T) {
	...
	priv, err := cryptoutil.GenerateKey(rand.Reader)
	if err != nil {
	...
	r, s, err := cryptoutil.Sign(rand.Reader, priv, hash)
	if err != nil {
		panic(err)
	}

	ret := cryptoutil.Verify(&priv.PublicKey, hash, r, s)
	fmt.Println(ret)
}
...
func BenchmarkSign(b *testing.B) {
	...
	priv, _ := cryptoutil.GenerateKey(rand.Reader)
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _ = cryptoutil.Sign(rand.Reader, priv, hashed[:])
	}
}
...
func TestSignAndVerify(t *testing.T) {
	...
	hash.Write(origin)
	hashed := hash.Sum(nil)
	r, s, err := cryptoutil.Sign(rand.Reader, priv, hashed)
	if err != nil {
		t.Errorf(" error signing: %s", err)
		return
	}

	if !cryptoutil.Verify(&priv.PublicKey, hashed, r, s) {
...
func TestKDF(t *testing.T) {
	...
	klen := 152
	actual := cryptoutil.keyDerivation(append(x2, y2...), klen)
...
func TestENC_GMT_EX1(t *testing.T) {
	...
	p256sm2CurveTest := cryptoutil.p256Curve{p256Sm2ParamsTest}

	cryptoutil.generateRandK = func (rand io.Reader, c elliptic.Curve) (k *big.Int) {
...
func TestCryptoToolCompare(t *testing.T) {
	...
	priv := &cryptoutil.PrivateKey{}
	priv.PublicKey.Curve = cryptoutil.P256Sm2()
	...
	cryptoutil.Encrypt(rand.Reader, msg, &priv.PublicKey)
}
...
func TestEnc(t *testing.T) {
	...
	enc, err := cryptoutil.Encrypt(rand.Reader, []byte(msg), &priv.PublicKey)
	if err != nil {
		t.Fatalf("encrypt failed : %s", err.Error())
	}
	dec,err := cryptoutil.Decrypt(enc, priv)
	if err != nil {
