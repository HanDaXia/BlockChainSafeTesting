...
import (
	...
	"github.com/HanDaXia/BlockChainSafeTesting/messagehub/cryptoutil/sm/sm3"
	"crypto/x509/pkix"
	"encoding/asn1"
	"encoding/pem"
	"errors"
...
func GetSignatureAlgorithmName(ai pkix.AlgorithmIdentifier) string {
	for _, details := range signatureAlgorithmDetails {
		if ai.Algorithm.Equal(details.oid) {
			return details.algo.String()
		}
	}
	return "unknown"
}
...
// add by syl, ParseTBS get tbs certificate info of the cert.
func (c *Certificate) GetTBS() (tbsCert TBSCertificate, err error) {
	_, err = asn1.Unmarshal(c.RawTBSCertificate, &tbsCert)
	return
}
...
// ParseCertificate parses a single certificate from the given ASN.1 DER data.
func ParsePem(pemBytes []byte) (*Certificate, error) {
	block, rest := pem.Decode(pemBytes)
	if block == nil {
		return nil, errors.New("pem decode error, bad format")
	}
