...
import (
	...
	"github.com/HanDaXia/BlockChainSafeTesting/messagehub/cryptoutil/crypto"
	"internal/testenv"
	"math/big"
	"net"
	"os/exec"
...
func TestParsePKCS1PrivateKey(t *testing.T) {
	...
	priv, err := cryptoutil.ParsePKCS1PrivateKey(block.Bytes)
	if err != nil {
	...
	if _, err := cryptoutil.ParsePKCS1PrivateKey(data); err == nil {
...
func TestParsePKIXPublicKey(t *testing.T) {
	...
	pub, err := cryptoutil.ParsePKIXPublicKey(block.Bytes)
	if err != nil {
	...
	pubBytes2, err := cryptoutil.MarshalPKIXPublicKey(rsaPub)
	if err != nil {
...
func init() {
	...
	var err error
	if testPrivateKey, err = cryptoutil.ParsePKCS1PrivateKey(block.Bytes); err != nil {
...
func TestMarshalRSAPrivateKey(t *testing.T) {
	...
	derBytes := cryptoutil.MarshalPKCS1PrivateKey(priv)

	priv2, err := cryptoutil.ParsePKCS1PrivateKey(derBytes)
	if err != nil {
...
func TestMatchHostnames(t *testing.T) {
	for i, test := range matchHostnamesTests {
		r := cryptoutil.matchHostnames(test.pattern, test.host)
		if r != test.ok {
...
func TestCertificateParse(t *testing.T) {
	...
	certs, err := cryptoutil.ParseCertificates(s)
	if err != nil {
		t.Error(err)
	}
	if len(certs) != 2 {
...
func TestCreateSelfSignedCertificate(t *testing.T) {
	...
	tests := []struct {
		...
		pub, priv interface{}
		checkSig  bool
		sigAlgo   cryptoutil.SignatureAlgorithm
	}{
		{"RSA/RSA", &testPrivateKey.PublicKey, testPrivateKey, true, cryptoutil.SHA1WithRSA},
		{"RSA/ECDSA", &testPrivateKey.PublicKey, ecdsaPriv, false, cryptoutil.ECDSAWithSHA384},
		{"ECDSA/RSA", &ecdsaPriv.PublicKey, testPrivateKey, false, cryptoutil.SHA256WithRSA},
		{"ECDSA/ECDSA", &ecdsaPriv.PublicKey, ecdsaPriv, true, cryptoutil.ECDSAWithSHA1},
		{"RSAPSS/RSAPSS", &testPrivateKey.PublicKey, testPrivateKey, true, cryptoutil.SHA256WithRSAPSS},
		{"ECDSA/RSAPSS", &ecdsaPriv.PublicKey, testPrivateKey, false, cryptoutil.SHA256WithRSAPSS},
		{"RSAPSS/ECDSA", &testPrivateKey.PublicKey, ecdsaPriv, false, cryptoutil.ECDSAWithSHA384},
	}

	testExtKeyUsage := []cryptoutil.ExtKeyUsage{cryptoutil.ExtKeyUsageClientAuth, cryptoutil.ExtKeyUsageServerAuth}
	testUnknownExtKeyUsage := []asn1.ObjectIdentifier{[]int{1, 2, 3}, []int{2, 59, 1}}
	extraExtensionData := []byte("extra extension")

	for _, test := range tests {
		commonName := "test.example.com"
		template := cryptoutil.Certificate{
			...
			crypto.SignatureAlgorithm: test.sigAlgo,

			SubjectKeyId:    []byte{1, 2, 3, 4},
			crypto.KeyUsage: cryptoutil.KeyUsageCertSign,

			crypto.ExtKeyUsage: testExtKeyUsage,
			...
			ExtraExtensions: []pkix.Extension{
				...
				{
					Id:       cryptoutil.oidExtensionSubjectKeyId,
					Critical: false,
		...
		derBytes, err := cryptoutil.CreateCertificate(random, &template, &template, test.pub, test.priv)
		if err != nil {
			t.Errorf("%s: failed to create certificate: %s", test.name, err)
			continue
		}

		cert, err := cryptoutil.ParseCertificate(derBytes)
		if err != nil {
...
var ecdsaTests = []struct {
	...
	{cryptoutil.ECDSAWithSHA1, ecdsaSHA1CertPem},
	{cryptoutil.ECDSAWithSHA256, ecdsaSHA256p256CertPem},
	{cryptoutil.ECDSAWithSHA256, ecdsaSHA256p384CertPem},
	{cryptoutil.ECDSAWithSHA384, ecdsaSHA384p521CertPem},
}

func TestECDSA(t *testing.T) {
	for i, test := range ecdsaTests {
		pemBlock, _ := pem.Decode([]byte(test.pemCert))
		cert, err := cryptoutil.ParseCertificate(pemBlock.Bytes)
		if err != nil {
		...
		if pka := cert.PublicKeyAlgorithm; pka != cryptoutil.ECDSA {
...
func TestParseCertificateWithDsaPublicKey(t *testing.T) {
	...
	if cert.PublicKeyAlgorithm != cryptoutil.DSA {
...
func TestParseCertificateWithDSASignatureAlgorithm(t *testing.T) {
	...
	if cert.SignatureAlgorithm != cryptoutil.DSAWithSHA1 {
...
func TestRSAPSSSelfSigned(t *testing.T) {
	...
	cert, err := cryptoutil.ParseCertificate(der.Bytes)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCRLCreation(t *testing.T) {
	block, _ := pem.Decode([]byte(pemPrivateKey))
	priv, _ := cryptoutil.ParsePKCS1PrivateKey(block.Bytes)
	block, _ = pem.Decode([]byte(pemCertificate))
	cert, _ := cryptoutil.ParseCertificate(block.Bytes)
	...
	parsedCRL, err := cryptoutil.ParseDERCRL(crlBytes)
	if err != nil {
...
func TestParseDERCRL(t *testing.T) {
	...
	certList, err := cryptoutil.ParseDERCRL(derBytes)
	if err != nil {
...
func TestParsePEMCRL(t *testing.T) {
	...
	certList, err := cryptoutil.ParseCRL(pemBytes)
	if err != nil {
		t.Errorf("error parsing: %s", err)
		return
	}
...
func TestCreateCertificateRequest(t *testing.T) {
	...
	tests := []struct {
		...
		{"RSA", testPrivateKey, cryptoutil.SHA1WithRSA},
		{"ECDSA-256", ecdsa256Priv, cryptoutil.ECDSAWithSHA1},
		{"ECDSA-384", ecdsa384Priv, cryptoutil.ECDSAWithSHA1},
		{"ECDSA-521", ecdsa521Priv, cryptoutil.ECDSAWithSHA1},
	}

	for _, test := range tests {
		template := cryptoutil.CertificateRequest{
			Subject: pkix.Name{
				CommonName:   "test.example.com",
				Organization: []string{"Σ Acme Co"},
			},
		...
		derBytes, err := cryptoutil.CreateCertificateRequest(random, &template, test.priv)
		if err != nil {
			t.Errorf("%s: failed to create certificate request: %s", test.name, err)
			continue
		}

		out, err := cryptoutil.ParseCertificateRequest(derBytes)
		if err != nil {
			t.Errorf("%s: failed to create certificate request: %s", test.name, err)
			continue
		}
...
func marshalAndParseCSR(t *testing.T, template *cryptoutil.CertificateRequest) *cryptoutil.CertificateRequest {
	derBytes, err := cryptoutil.CreateCertificateRequest(rand.Reader, template, testPrivateKey)
	if err != nil {
		t.Fatal(err)
	}

	csr, err := cryptoutil.ParseCertificateRequest(derBytes)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCertificateRequestOverrides(t *testing.T) {
	sanContents, err := cryptoutil.marshalSANs([]string{"foo.example.com"}, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	template := cryptoutil.CertificateRequest{
		...
		ExtraExtensions: []pkix.Extension{
			{
				Id:    cryptoutil.oidExtensionSubjectAltName,
				Value: sanContents,
			},
		},
	}
	...
	template.Attributes = []pkix.AttributeTypeAndValueSET{
		{
			Type: cryptoutil.oidExtensionRequest,
			Value: [][]pkix.AttributeTypeAndValue{
				{
					{
						Type:  cryptoutil.oidExtensionAuthorityInfoAccess,
						Value: []byte("foo"),
					},
				},
			},
	...
	if !csr.Attributes[0].Type.Equal(cryptoutil.oidExtensionRequest) ||
		len(csr.Attributes[0].Value) != 1 ||
		len(csr.Attributes[0].Value[0]) != 2 {
		t.Errorf("bad attributes: %#v\n", csr.Attributes)
	}

	sanContents2, err := cryptoutil.marshalSANs([]string{"foo2.example.com"}, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	template.Attributes[0].Value[0] = append(template.Attributes[0].Value[0], pkix.AttributeTypeAndValue{
		Type:  cryptoutil.oidExtensionSubjectAltName,
		Value: sanContents2,
	})
...
func TestParseCertificateRequest(t *testing.T) {
	for _, csrBase64 := range csrBase64Array {
		...
		csr, err := cryptoutil.ParseCertificateRequest(csrBytes)
		if err != nil {
		...
		for _, e := range csr.Extensions {
			if e.Id.Equal(cryptoutil.oidExtensionBasicConstraints) {
				found = true
				break
			}
		}
...
func TestCriticalFlagInCSRRequestedExtensions(t *testing.T) {
	...
	expected := []struct {
		...
		{cryptoutil.oidExtensionBasicConstraints, fromBase64("MAYBAf8CAQA=")},
		{cryptoutil.oidExtensionKeyUsage, fromBase64("AwIChA==")},
	}
...
func serialiseAndParse(t *testing.T, template *cryptoutil.Certificate) *cryptoutil.Certificate {
	derBytes, err := cryptoutil.CreateCertificate(rand.Reader, template, template, &testPrivateKey.PublicKey, testPrivateKey)
	if err != nil {
...
func TestASN1BitLength(t *testing.T) {
	...
	for i, test := range tests {
		if got := cryptoutil.asn1BitLength(test.bytes); got != test.bitLen {
...
func TestVerifyEmptyCertificate(t *testing.T) {
	if _, err := new(cryptoutil.Certificate).Verify(cryptoutil.VerifyOptions{}); err != cryptoutil.errNotParsed {
		t.Errorf("Verifying empty certificate resulted in unexpected error: %q (wanted %q)", err, cryptoutil.errNotParsed)
	}
}

func TestInsecureAlgorithmErrorString(t *testing.T) {
	tests := []struct {
		sa   cryptoutil.SignatureAlgorithm
		want string
	}{
		{cryptoutil.MD2WithRSA, "x509: cannot verify signature: insecure algorithm MD2-RSA"},
	...
	for i, tt := range tests {
		if got := fmt.Sprint(cryptoutil.InsecureAlgorithmError(tt.sa)); got != tt.want {
			t.Errorf("%d. mismatch.\n got: %s\nwant: %s\n", i, got, tt.want)
		}
	}
}
...
func TestMD5(t *testing.T) {
	...
	if sa := cert.SignatureAlgorithm; sa != cryptoutil.MD5WithRSA {
		t.Errorf("signature algorithm is %v, want %v", sa, cryptoutil.MD5WithRSA)
	}
	if err = cert.CheckSignatureFrom(cert); err == nil {
		t.Fatalf("certificate verification succeeded incorrectly")
	}
	if _, ok := err.(cryptoutil.InsecureAlgorithmError); !ok {
...
func TestRSAMissingNULLParameters(t *testing.T) {
	...
	if _, err := cryptoutil.ParseCertificate(block.Bytes); err == nil {
...
func TestISOOIDInCertificate(t *testing.T) {
	...
	if cert, err := cryptoutil.ParseCertificate(block.Bytes); err != nil {
		t.Errorf("certificate with ISO OID failed to parse: %s", err)
	} else if cert.SignatureAlgorithm == cryptoutil.UnknownSignatureAlgorithm {
...
func TestMultipleRDN(t *testing.T) {
	...
	cert, err := cryptoutil.ParseCertificate(block.Bytes)
	if err != nil {
...
func TestSystemCertPool(t *testing.T) {
	...
	_, err := cryptoutil.SystemCertPool()
	if err != nil {
		t.Fatal(err)
	}
}
