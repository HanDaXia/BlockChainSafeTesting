...
func TestDecrypt(t *testing.T) {
	for i, data := range testData {
		...
		der, err := cryptoutil.DecryptPEMBlock(block, data.password)
		if err != nil {
			t.Error("decrypt failed: ", err)
			continue
		}
		if _, err := cryptoutil.ParsePKCS1PrivateKey(der); err != nil {
...
func TestEncrypt(t *testing.T) {
	for i, data := range testData {
		...
		block, err := cryptoutil.EncryptPEMBlock(rand.Reader, "RSA PRIVATE KEY", plainDER, password, data.kind)
		if err != nil {
			t.Error("encrypt: ", err)
			continue
		}
		if !cryptoutil.IsEncryptedPEMBlock(block) {
		...
		der, err := cryptoutil.DecryptPEMBlock(block, password)
		if err != nil {
			t.Error("decrypt: ", err)
			continue
		}
...
var testData = []struct {
	kind     cryptoutil.PEMCipher
	password []byte
	pemData  []byte
	plainDER string
}{
	{
		kind:     cryptoutil.PEMCipherDES,
		password: []byte("asdf"),
		pemData: []byte(`
...
tZZZxCtPAm7shftEib0VU77Lk8MsXJcx2C4voRsjEw==`,
	...
	{
		kind:     cryptoutil.PEMCipherAES128,
		password: []byte("asdf"),
		pemData: []byte(`
-----BEGIN RSA PRIVATE KEY-----
Proc-Type: 4,ENCRYPTED
...
B3WlRNTXR2WsJ5JdByezg9xzdXzULqmga0OE339a`,
	...
	{
		kind:     cryptoutil.PEMCipherAES192,
		password: []byte("asdf"),
		pemData: []byte(`
-----BEGIN RSA PRIVATE KEY-----
Proc-Type: 4,ENCRYPTED
...
+qGKsd09NhQ/G27y/DARzOYtml1NvdmCQAgsDIIOLA==`,
	...
	{
		kind:     cryptoutil.PEMCipherAES256,
		password: []byte("asdf"),
		pemData: []byte(`
-----BEGIN RSA PRIVATE KEY-----
Proc-Type: 4,ENCRYPTED
...
func TestIncompleteBlock(t *testing.T) {
	...
	_, err := cryptoutil.DecryptPEMBlock(block, []byte("foo"))
	if err == nil {
