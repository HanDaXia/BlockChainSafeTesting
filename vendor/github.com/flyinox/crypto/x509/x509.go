...
func GetSignatureAlgorithmFromOID(algoOid asn1.ObjectIdentifier) SignatureAlgorithm {
	for _, algo := range signatureAlgorithmDetails {
		if algoOid.Equal(algo.oid) {
			return algo.algo
		}
	}
...
func CheckSignatureNor(algo SignatureAlgorithm, signed, signature []byte, pub interface{}) error {
	return checkSignature(algo, signed, signature, pub)
}
