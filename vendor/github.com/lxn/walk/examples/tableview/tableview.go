...
func (m *FooModel) Sort(col int, order walk.SortOrder) error {
	...
	sort.SliceStable(m.itemsss, func(i, j int) bool {
		a, b := m.itemsss[i], m.itemsss[j]
...
func (m *FooModel) ResetRows() {
	...
	m.itemsss = make([]*Foo, rand.Intn(50000))

	now := time.Now()

	for i := range m.itemsss {
		m.itemsss[i] = &Foo{
			Index: i,
