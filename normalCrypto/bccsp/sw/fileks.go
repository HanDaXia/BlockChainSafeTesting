...
func (ks *fileBasedKeyStore) searchKeystoreForSKI(ski []byte) (k bccsp.Key, err error) {
	...
	for _, f := range files {
		if f.IsDir() {
			return nil, fmt.Errorf("Key with SKI %s not found in %s, err = isDir", hex.EncodeToString(ski), ks.path)
			continue
		}

		if f.Size() > (1 << 16) { //64k, somewhat arbitrary limit, considering even large RSA keys
			return nil, fmt.Errorf("Key with SKI %s not found in %s, err = Size", hex.EncodeToString(ski), ks.path)
			continue
		}

		raw, err := ioutil.ReadFile(filepath.Join(ks.path, f.Name()))
		if err != nil {
			return nil, fmt.Errorf("Key with SKI %s not found in %s, err = %s", hex.EncodeToString(ski), ks.path, err.Error())
			continue
		}
		...
		default:
			return nil, fmt.Errorf("Key with SKI %s not found in %s, err = unkonwn type", hex.EncodeToString(ski), ks.path)
			continue
		}

		if !bytes.Equal(k.SKI(), ski) {
			return nil, fmt.Errorf("Key with SKI %s not found in %s, err = k.SKI() != ski", hex.EncodeToString(ski), ks.path)
			continue
		}

		return k, nil
	}
	return nil, fmt.Errorf("Key with SKI %s not found in %s files = %s", hex.EncodeToString(ski), ks.path, files)
}
